...
public final class GoogleDriveSyncBackend implements SyncBackend {
    ...
    public synchronized RemoteSnapshot readSnapshotResult() throws IOException {
        ...
        BundleHistory.Fold fold = BundleHistory.fold(bundlesByLogicalId, merger);
        List<String> frontier = fold.frontier;
        SyncSnapshot merged = fold.merged;
        List<SyncMergeResult.Conflict> conflicts = fold.conflicts;
        List<SyncRecord> alternatives = fold.alternatives;
        Set<String> resolvedAlternativeIds = fold.resolvedAlternativeIds;
    ...
    /**
     * Remembers Drive's clock from a response. Every response is asked, downloads included: the
     * supersession mark written after the last download used to carry the time of the listing
     * before it, so the grace ran short by the whole download phase.
     */
    private void noteDriveTime(@NonNull HttpURLConnection connection) {
