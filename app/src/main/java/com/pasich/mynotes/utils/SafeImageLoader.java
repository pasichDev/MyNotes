 ...
 * SafeImageLoader
 * <p>
 * Safely load large images from disk/URI without OutOfMemory.
 * Can:
 * • read bounds without decoding (to determine size)
 * • scale via inSampleSize
 * • correct EXIF rotations
 */
public class SafeImageLoader {
     ...
     * <p>
     * Steps:
     * 1) Read only the dimensions (inJustDecodeBounds=true) to determine the optimal scale.
     * 2) Calculate inSampleSize so as not to load the photo in full size (saving RAM).
     * 3) Decode the actual image with the desired scaling.
     * 4) Read EXIF and return the image in the correct orientation.
     *
     * @param ctx     context
     * @param uri     file Uri
     ...
     * <p>
     * Algorithm:
     * • Divide the dimensions by 2 until they fit into targetW/targetH.
    ...
    /**
     * Applies EXIF rotation to Bitmap.
