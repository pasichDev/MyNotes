...
public class NotificationChannelManager {
    ...
    /**
     * Idempotent: creates the current versioned channel if it does not exist.
     * On first call after update from old version, deletes the legacy "reminders" channel.
     */
    ...
    /**
     * Deletes the current versioned channel, bumps the version, creates the new channel
     * with newSoundUri. Pass null to reset to the system default notification sound.
     */
    public static void changeSound(Context ctx, NotificationPreferencesCache prefs, Uri newSoundUri) {
    ...
    private static void createChannel(Context ctx, NotificationManager nm,
                                      NotificationPreferencesCache prefs) {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.O) return;
