 ...
 *  - byte[] → byte[] processing (no I/O inside)
 *  - very fast path (no resizing, no EXIF parsing, no metadata work)
 * <p>
 * Behavior:
 *  - Detects input type by magic bytes (JPEG / PNG / WEBP)
 *  - PNG with transparency → stays PNG
 *  - PNG without transparency → converted to JPEG (smaller)
 *  - JPEG stays JPEG
 *  - WEBP stays WEBP (WEBP_LOSSY on Android R+)
 *  - UNKNOWN formats (HEIC, GIF, PDF, TXT, ZIP, etc.) → returned unchanged
 * <p>
 * Quality:
 *  - extraOptimize = true  → strong compression (~60)
 *  - extraOptimize = false → normal compression (~85)
 */
public class ImageOptimizer {
     ...
     * <p>
     * - extraOptimize = true  → lower quality (≈60)
     * - extraOptimize = false → higher quality (≈85)
     * <p>
     * PNG with alpha → stays PNG.
     * PNG without alpha → JPEG.
     * JPEG stays JPEG.
     * WEBP stays WEBP (lossy if Android R+).
     *
