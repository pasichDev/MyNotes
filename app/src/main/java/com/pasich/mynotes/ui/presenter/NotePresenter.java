...
public class NotePresenter extends BasePresenter<NoteContract.view>
                ...
                .add(
                        autoSaveTrigger
                                ...
                                .subscribe(
                                        ...
                                        error ->
                                                android.util.Log.e(
                                                        TAG, "autoSave stream error", error)));
    }
     ...
     *
     * <p>Logic: 1) If the note is empty, do not save anything (IDLE). 2) Cancel the previous
     * auto-save if the user continues to enter text. 3) Set status to PENDING — there are unsaved
     * changes. 4) After a pause (AUTO_SAVE_DELAY), execute saveNote(). - onSuccess → SAVED → after
     * 3 seconds → IDLE - onError → ERROR → after 5 seconds → PENDING
     *
     * <p>Works as “smart” autosave: saves only after a pause in typing.
     */
    @Override
     ...
     * <p>Main logic:
     *
     * <p>1) Null check: - If the note is missing, saving is impossible, so we return an error.
     *
     * <p>2) Activity lifecycle check: - If View is destroyed, but there is data in the note —
     * emergency saving is performed, which works without UI and without callback.
     *
     * <p>3) Content check: - If the note is empty (title/value/valueJson), there is no point in
     * saving. We return IDLE and success.
     *
     * <p>4) Checking for changes: - If the note has not changed compared to the last saved version
     * — we do not perform unnecessary writing to the database.
     *
     * <p>5) Main saving: - We update the edit date. - Perform updateNote() via RxJava (IO → UI). -
     * After successful saving, update lastSavedXXX so that the system correctly identifies the
     * changed data next time.
     *
     * <p>6) Error handling: - In case of failure — log and call callback.onError().
     *
     * <p>Note: The method is only responsible for updating an existing note.
     */
    ...
    /**
     * Emergency saving when Activity is destroyed but there is unsaved data. Performed
    ...
    /**
     * Updates the save status and notifies the UI. If the save is complete and closing is expected,
     * closes the screen.
     */
    ...
    /**
     * Handles closing the note screen: - deletes empty new notes, - saves pending changes, - or
     * closes immediately if nothing to save.
     */
    @Override
    ...
    /**
     * Loads parameters passed to the Activity via Intent. Initializes presenter fields (id, tag,
     * shared text, creation flag). If this is a new note, creates a fresh Note instance with
     * default values.
     */
    @Override
    ...
    /**
     * Loads a note from the database by its ID. Once received, forwards it to the View, updates the
     * internal targetNote, synchronizes the savedNote snapshot, and resets the save state to IDLE.
     */
    @Override
     ...
     *
     * <p>Used before any saving operations: if the Activity/Fragment no longer exists
     */
     ...
     *
     * <p>If title is not null → updates the title. If value is not null → updates the plain text
     * and clears valueJson.
     *
     * <p>Marks the note as non-rich and triggers auto-save. Passing null means "do not modify this
     * field".
     */
    @Override
