...
public class PopupWindowsTag {
    ...
    public PopupWindowsTag(
            LayoutInflater layoutInflater,
            View anchor,
            Tag tag,
            PopupWindowsTagOnClickListener listener) {
        this.mTag = tag;
        ...
        if (mAnchor.getX() > widthDisplayCenter) {
            mBinding.getRoot()
                    .setBackground(
                            ContextCompat.getDrawable(
                                    ...
                                    R.drawable.background_popup_tag_right));
            ...
            mBinding.getRoot()
                    .setBackground(
                            ContextCompat.getDrawable(
                                    mBinding.getRoot().getContext(),
                                    R.drawable.background_popup_tag_left));
            xof = widthAnchor / 3;
        }
    ...
    private void editVisibleConfiguration() {
        mBinding.imageTagVisible.setImageResource(
                mTag.getVisibility() == 1 ? R.drawable.ic_show : R.drawable.ic_hide);
        mBinding.textVisibilityTag.setText(
                mTag.getVisibility() == 1 ? R.string.visibleTag : R.string.hiddeTag);
    }

    private void onVibrate() {
        Vibrator vibrator =
                (Vibrator)
                        mBinding.getRoot().getContext().getSystemService(Context.VIBRATOR_SERVICE);
    ...
    private void initListeners() {
        mBinding.deleteTag.setOnClickListener(
                v -> {
                    mListener.deleteTag();
                    getPopupWindows().dismiss();
                });
        mBinding.renameTag.setOnClickListener(
                v -> {
                    mListener.renameTag();
                    getPopupWindows().dismiss();
                });
        mBinding.visibleTag.setOnClickListener(
                v -> {
